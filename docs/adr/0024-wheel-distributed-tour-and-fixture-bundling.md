...
- **Status:** Accepted
- **Partially superseded by:** [ADR 0035 (CLI rename: walkthrough + fitting-room)](0035-cli-rename-walkthrough-and-fitting-room-and-recipient-experience-naming-principle.md) (2026-05-14) — the CLI verb name (`tailor tour`) was renamed to `tailor fitting-room` per the recipient-experience-shaped naming principle. The wheel-distribution shape, synthetic-by-construction precondition, bundled-fixture invariants, and the `_VARIANT_FIXTURES` table are all preserved unchanged. The CLI alias `tailor tour` and the module `tailor.tour` (as a re-export shim) survive through v7.1.0 with deprecation hints; both are removed in v7.2.0.
- **Partially superseded by:** [ADR 0040 (Bounded setup-time conductor surface)](0040-bounded-setup-time-conductor-surface.md) (v8.0.0, 2026-05-19) — the `tailor fitting-room` CLI command (renamed from `tailor tour` by ADR 0035) was **hard-removed** in v8.0.0 with no deprecation shim, replaced by the `FittingRoomLayer` MCP tools. The wheel-distribution shape, synthetic-by-construction precondition, and bundled-fixture invariants this ADR codified are retained inside the new MCP layer; only the CLI verb was retired.
- **Date:** 2026-05-04
...
- **Amended:** 2026-05-14 — CLI verb name only: `tailor tour` → `tailor fitting-room` per [ADR 0035](0035-cli-rename-walkthrough-and-fitting-room-and-recipient-experience-naming-principle.md). The substance of this ADR (wheel distribution, synthetic-by-construction precondition, fixture bundling, variant table) is fully retained. Prose references to "tour" in this ADR's body remain as historical record per the project's historical-preservation rule; the operational verb in newly-written code and docs is `fitting-room`.
...
**Retirement closeout (2026-05-09 — see [ADR 0032](0032-retire-public-mirror-distribution.md)):** the public-mirror carve-out codified in this § 3.1 is retired. Wheel-handoff via personal email (the original § 3 channel) becomes the sole friend-shareable distribution path through Phase 1; GitHub Pages on the now-public source repo at Phase 2 PyPI publish supersedes from there. The structural argument the boss-architect surfaced on 2026-05-09 — *"the public mirror is unneeded at this point since I can just send a wheel file from the private repo to anyone up until phase 1 when the pages become unnecessary anyways"* — names the redundancy directly: interim infrastructure scheduled for its own obsolescence under the ROADMAP § Phase 2 commitment. The mirror repo `saahasmuthineni/biosensormcpdemo` was archived (not deleted) via `gh repo archive` so prior friend-shares with the legacy `https://saahasmuthineni.github.io/biosensormcpdemo/` URL continue to resolve indefinitely; the v6.13.0 snapshot remains visible. ADR 0032 retains ADR 0030's zero-outbound-affordances rendering invariant, which now governs wheel-handoff render output rather than the retired Pages mirror. The three reversal conditions named above (source private, synthetic-by-construction, handful-recipient scope) stand unchanged for the surviving wheel-by-email channel — they were always carve-out invariants, not mirror-specific ones.
