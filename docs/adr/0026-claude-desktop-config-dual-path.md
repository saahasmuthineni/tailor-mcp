...
- **Amended:** 2026-05-14 — confirmation footer per [ADR 0035](0035-cli-rename-walkthrough-and-fitting-room-and-recipient-experience-naming-principle.md). The v7.1.0 rename of `tailor tour` → `tailor fitting-room` changes the Claude Desktop registration key from `tailor-tour-{variant}` to `tailor-fitting-room-{variant}`. The existing `_is_orphan_entry_key` matcher (`key == "tailor" or key.startswith("tailor-") or key.startswith("biosensor-")`) covers the new key without modification — `tailor-fitting-room-{variant}` matches the `tailor-*` prefix. The v6.10.4 invariant ("after a successful fitting-room --force, exactly one `tailor-*` entry exists in each detected Claude Desktop config") is preserved verbatim; the matched prefix set is unchanged. No code change required by this amendment.
