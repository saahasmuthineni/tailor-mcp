...
## Amendment 2026-05-13 — URL allowlist tightens to zero outbound after PyPI publish

v7.0.13 ships PyPI publish for `tailor-mcp` (`uv tool install tailor-mcp`). The wheel-by-email distribution channel that ADR 0032 inherited from ADR 0024 § 3.1 is superseded by PyPI for the dominant install path. The source repo stays private (per v7.0.13's unbundling decision, repo public-flip is deferred to a future version under a three-condition trigger) and no GitHub releases will be published against it. The wheel-release-asset URL pattern `^https://github.com/[^/]+/[^/]+/releases/download/` therefore has no canonical instance after v7.0.13.

The URL allowlist tightens to zero outbound URLs in public mode. The rendering invariant — *"the public-mode page contains no outbound contact mechanism"* — is preserved in spirit; what changes is the carve-out for the wheel-release-asset URL, which is removed structurally because no referent remains. A future contributor adding any outbound link hits a CI failure regardless of whether it's a Discord, a mailto, or a github-releases URL.

Implementation: `src/tailor/demo/runner.py:_enforce_public_url_allowlist` rejects every outbound URL; the `_ALLOWED_PUBLIC_URL_PATTERN` regex is retired. The `install_url_base` parameter on `_generate_shareable_markdown` is retired along with the `TAILOR_DEMO_INSTALL_URL_BASE` env-var override. Regression test at `tests/test_demo_runner.py:test_public_mode_url_allowlist_rejects_wheel_release_asset_after_tightening` defends the new posture.

The reversal condition stated above is unchanged in shape. Clause (a) — *PyPI publication becoming the right answer per Alternative 1* — is satisfied by v7.0.13's publish, but PyPI publication reinforced the zero-outbound-affordances rule rather than reversing it. Clause (b) — *boss opts into community-shape distribution* — still requires a superseding ADR with written reversal note before any contact-mechanism returns to the public-mode page.
