...
| Default config dir | **`~/.tailor/`** | `~/.biosensor-mcp/` | Mirrors the new project identity; migration handled via `tailor migrate` |
...
4. **PyPI install command**: `uv tool install git+https://github.com/saahasmuthineni/Biosensor-to-LLM-Connector.git` (still works because the GitHub repo URL is unchanged) and `pip install tailor-mcp` (when published) replace the v6 install commands in docs.
...
- The `tailor migrate` subcommand is a recipient-friendly upgrade path that the v6.10.x patch quartet's failure modes argued for: *"upgrades that silently abandon user data are a class of recipient bug we want closed at the framework level, not via docs."*
