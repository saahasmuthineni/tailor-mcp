...
3. **`examples/cohort_demo/beta/vault/moments/2026-05-05-biosensor-mcp-6-9-0-...md`** — a captured vault moment from 2026-05-05. Vault moments are immutable historical artifacts; the filename's `biosensor-mcp` substring documents the project's name at the moment of capture.
...
- **The first deployment recipe (demo cohort researcher first-look) is unchanged.** The bundled fixtures, the `tailor tour` command, the `tailor demo` command, the demo runner output — all carry the new naming, but the recipe's content (force / EMG / 31P-MRS cohort analysis on 16 synthetic subjects) is what it was. Future deployment recipes compose on the same engine without disturbing this one.
