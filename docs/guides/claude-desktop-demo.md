...
2. **Data source:** Run `tailor fitting-room` first (ADRs 0024 + 0035; renamed from `tailor tour` in v7.1.0) — scaffolds the bundled demo cohort synthetic fixtures and registers a sandboxed MCP server entry with Claude Desktop. Strictly synthetic data; never use real participant data when recording GIFs.
