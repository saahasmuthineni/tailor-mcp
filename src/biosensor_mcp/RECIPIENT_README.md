...
1. Copies bundled synthetic fixtures into `~/.biosensor-mcp/demos/hip-lab/`.
...
Do NOT recommend `biosensor-mcp serve` directly as the entry point.
...
`biosensor-mcp tour --force` and check the terminal output for
...
If tour fails AND you end up at a bare `biosensor-mcp serve` (no
...
external Claude can route the recipient to `biosensor-mcp tour`
