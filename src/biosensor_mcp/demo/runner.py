...
``biosensor-mcp demo`` exercises the framework's load-bearing
...
the actual output of `biosensor-mcp demo` on v{version}; running the
...
```
uvx --from {wheel_url} biosensor-mcp demo
```

Or with `pipx`:

```
pipx run --spec {wheel_url} biosensor-mcp demo
```
...
Generated by `biosensor-mcp demo --save-shareable` on v{version}.
"""
...
def run_demo(
    *,
    ...
    import logging
    logging.getLogger("biosensor-mcp").setLevel(logging.ERROR)
    ...
    # `biosensor-mcp demo` (without the flag) is behaviourally
    # unchanged.
