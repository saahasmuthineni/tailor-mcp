...
diagnostic tool when ``biosensor-mcp serve`` is started without the
...
``biosensor-mcp serve`` directly (via web-Claude-mediated manual
config rather than the canonical ``biosensor-mcp tour``), and now
...
log = logging.getLogger("biosensor-mcp.setup_help")
...
_SETUP_HELP_DESCRIPTION = (
    ...
    "the recipient to scaffold the HIP Lab demo via `biosensor-mcp tour`, "
...
class SetupHelpLayer:
    ...
    async def execute(self, tool_name: str, params: dict) -> dict:
        ...
        default_tour_target = (
            Path.home() / ".biosensor-mcp" / "demos" / "hip-lab"
        )
        tour_target_exists = default_tour_target.exists()

        return {
            "diagnosis": (
                "This biosensor-mcp server is running but no demo "
                "scaffold has been installed. The expected recipient "
                "path is `biosensor-mcp tour`, which copies bundled "
            ...
            "recipient_steps": [
                ...
                "macOS).",
                "Run: biosensor-mcp tour",
                "If that errors with 'tour scaffold already exists', "
                "run: biosensor-mcp tour --force",
            ...
            "if_tour_keeps_failing": (
                ...
                "`biosensor-mcp tour` to the project owner. The most "
