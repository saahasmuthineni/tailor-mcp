...
known-debt). The CLI verb that drives this module is ``tailor
walkthrough``; ``tailor demo`` remains as a one-cycle deprecation
alias through v7.1.0.

Per ADR 0029 (which partially supersedes ADR 0027 § Negative
consequences "the demo bypasses RouterMCP by design"),
``tailor walkthrough`` exercises the framework's load-bearing
...
def _generate_shareable_markdown(
    ...
    if audience == "public":
        ...
        rendered = (
            ...
            f"`tailor walkthrough` on v{version}; running the same\n"
            ...
            "\n"
            "```\n"
            "uvx --from tailor-mcp tailor walkthrough\n"
            "```\n"
            "\n"
            "Or with `pipx`:\n"
            "\n"
            "```\n"
            "pipx run --spec tailor-mcp tailor walkthrough\n"
            "```\n"
            "\n"
            ...
            "---\n"
            "\n"
            f"Generated by `tailor walkthrough --audience=public "
...
the actual output of `tailor walkthrough` on v{version}; running the
...
Generated by `tailor walkthrough --save-shareable` on v{version}.
"""
...
def run_demo(
    *,
    ...
    if audience == "developer":
        ...
        print(
            "  - `tailor fitting-room` - scaffolds bundled fixtures into"
        )
        print(
    ...
    if save_shareable_path is None:
        ...
        print(
            "  `tailor walkthrough --save-shareable`"
        )
        print(
