...
``tailor.demo.runner`` is deliberately unchanged (internal Python
package structure is not recipient-facing; rename deferred as
known-debt). The ``tailor walkthrough`` CLI verb that drove this
module (and its ``tailor demo`` alias) was hard-removed in v8.0.0
per ADR 0040; the recipient path is the ``tailor_walkthrough_section``
MCP tool, and this module remains the library + ``python -m
tailor.demo`` developer path.

Per ADR 0029 (which partially supersedes ADR 0027 § Negative
consequences "the demo bypasses RouterMCP by design"),
the walkthrough exercises the framework's load-bearing
...
def _generate_shareable_markdown(
    ...
    Args:
        transcript: The captured stdout of a walkthrough run
    ...
    if audience == "public":
        ...
        rendered = (
            f"# Tailor — walkthrough (v{version})\n"
            "\n"
            ...
            "The transcript below is the actual output of Tailor's\n"
            f"walkthrough (`python -m tailor.demo`) on v{version};\n"
            "running the same command on your machine should produce\n"
            "structurally-identical output (Section 1's numerical\n"
            "claims are bit-identical by design).\n"
            "\n"
            ...
            "\n"
            "```\n"
            "uv run --with tailor-mcp python -m tailor.demo\n"
            "```\n"
            "\n"
            "Or with `pip` in any Python 3.10+ environment:\n"
            "\n"
            "```\n"
            "pip install tailor-mcp && python -m tailor.demo\n"
            "```\n"
            "\n"
            "The `uv` form installs the wheel into an ephemeral\n"
            "isolated env, runs the walkthrough (~30 seconds), and exits\n"
            "clean. Nothing persists on your machine.\n"
            "\n"
            "## Walkthrough output\n"
            "\n"
            f"{transcript_block}\n"
            ...
            "---\n"
            "\n"
            f"Generated by `python -m tailor.demo --audience=public "
    ...
    return f"""# Tailor - walkthrough (v{version})
...
the actual output of Tailor's walkthrough (`python -m tailor.demo`)
on v{version}; running the same command on your machine should
produce structurally-identical output (Section 1's numerical claims
...
The `uv` form installs the wheel into an ephemeral isolated env,
runs the walkthrough (~30 seconds), and exits clean. Nothing
...
Generated by `python -m tailor.demo --save-shareable` on v{version}.
"""
...
def run_demo(
    *,
    ...
    # save_shareable_path is None this is a no-op — a plain
    # walkthrough run (without the flag) is behaviourally
    # unchanged.
    ...
    if audience == "developer":
        ...
        print(
            "  - in Claude Desktop chat, ask to set up the demo"
        )
        print(
            "    fitting room (the tailor_fitting_room_scaffold tool"
        )
        print(
            "    scaffolds bundled fixtures into a durable directory)"
        )
    ...
    if save_shareable_path is None:
        ...
        print(
            "  `python -m tailor.demo --save-shareable`"
        )
        print(
            "(emits to ~/.tailor/shareable-walkthrough-vX.Y.Z.md by"
        )
        print(
