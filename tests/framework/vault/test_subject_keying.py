...
- Theme/moment subject_id round-trips through frontmatter + index.
- Set-once invariant on theme subject (reassignment is a hard error).
- list_notes / list_themes filter rows match-or-NULL when subject_id
...
class TestThemeSubjectRoundTrip:
    """Theme writes/reads with subject_id round-trip frontmatter + index."""
    ...
    def test_new_theme_with_subject_stamps_frontmatter(self):
        ...
        try:
            ...
            assert 'subject_id: "P004"' in content
    ...
    def test_theme_subject_persists_through_frontmatter_merge(self):
        """A second upsert (e.g. status update) must preserve subject_id."""
        vault_path, _data_path, layer, cleanup = _setup()
        try:
            _run(layer.execute("vault_upsert_theme", {
                "slug": "drift-p007",
            ...
            # Second call: status flip, no subject_id passed
            _run(layer.execute("vault_upsert_theme", {
                "slug": "drift-p007",
                "status": "resolved",
            ...
            assert 'subject_id: "P007"' in content
        finally:
            cleanup()
...
class TestSubjectFilteredQueries:
    ...
    def test_list_themes_filter_returns_matching_and_unscoped(self):
        ...
        try:
            ...
            res = _run(layer.execute("vault_list_themes", {"subject_id": "P004"}))
            ...
            res = _run(layer.execute("vault_list_themes", {"subject_id": "P007"}))
            slugs = {t["slug"] for t in res["themes"]}
...
class TestRescanSubjectBackfill:
    """Existing v6.1 vault notes pick up subject_id on next rescan."""

    def test_rescan_backfills_subject_from_frontmatter(self):
        vault_path, _data_path, layer, cleanup = _setup()
        try:
            # Hand-write a theme file with subject_id in frontmatter — simulating
            ...
            res = _run(layer.execute("vault_list_themes", {"subject_id": "P012"}))
            slugs = {t["slug"] for t in res["themes"]}
